...
namespace WindowsFormsApplication1
{
    ...
    public class DemoUtils
    {
        ...
        public static void BulkReadRenderResult(HslCommunication.Core.IReadWriteNet readWrite, string addTextBox, string lengthTextBox, string resultTextBox)
        {
            try
            {
                OperateResult<byte[]> read = readWrite.Read(addTextBox, ushort.Parse(lengthTextBox));
                if (read.IsSuccess)
                {
                    resultTextBox =  HslCommunication.BasicFramework.SoftBasic.ByteToHexString(read.Content);
                }
                else
                {
                    MessageBox.Show("Read Failed：" + read.ToMessageShowString());
